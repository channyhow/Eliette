...
import {
  JLALBANY_IMAGE_MAP,
  SASHAYOGAFLOW_IMAGE_MAP,
  JLALBANY_LOGO_MAP,
  SASHAYOGAFLOW_LOGO_MAP,
} from "../../utils/imageMap";
...
function Cards({ data }: CardProps) {
  ...
  const activeImageMap =
    activeItem?.id === "01" ? JLALBANY_IMAGE_MAP : SASHAYOGAFLOW_IMAGE_MAP;
  ...
  const handleNext = () =>
    setActiveItemIndex((prevIndex) => Math.min(prevIndex + 1, data.length - 1));
